...
def collect_user_predictions(
    ...
    """
    Run the model on the test set and collect predicted and true labels per user.

    Each batch from `test_loader` is expected to be a dict with keys:
    - "users": user IDs
    - "items": item IDs
    - "labels": true label values (e.g., ratings, clicks, scores, etc.)
    ...
    loader
        DataLoader yielding test batches as dicts with "users", "items", "labels".
    device
        Device on which to run inference (e.g., torch.device("cuda") or
        torch.device("cpu")).
    verbose
        If True, prints predictions and labels per batch.
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_label = model(users, items)
            true_label = labels.view(labels.size(0), -1).to(torch.float32)

            for idx in range(len(users)):
                user_id = users[idx].item()
                item_id = items[idx].item()
                pred = pred_label[idx][0].item()
                true = true_label[idx][0].item()
...
def compute_metrics(
    ...
    ----------
    user_pred_true
        Dictionary {user_id: [(pred_label, true_label), ...]}.
    metrics
    ...
    k
        The rank cutoff.
    threshold
        The label threshold for binary relevance.
    ...
    for _, user_labels in user_pred_true.items():

        # --- RMSE Calculation (Uses ALL items, no sorting needed) ---
        if "rmse" in metrics:
            sq_err, count = calc_squared_error(user_labels)
        ...
        if len(ranking_sums) > 0:
            ...
            # Sort once per user
            sorted_labels = sorted(user_labels, key=lambda x: x[0], reverse=True)
            ...
            if "recall" in metrics or "ndcg" in metrics:
                n_rel_total = sum(true_r >= threshold for (_, true_r) in sorted_labels)
